...
class DataMigration:
    ...
    def migrate_goals(self) -> None:
        ...
        if not file_path.exists():
            ...
            alt_paths = [
                self.state_dir / "goal_manager.json",
                self.state_dir / "goals_data.json"
            ]
        ...
        if not file_path or not file_path.exists():
            logger.warning(f"Goals file not found")
            return
    ...
    def migrate_observations(self) -> None:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                # Handle different possible structures
                observations = data.get('observations', data.get('observation_list', data.get('items', [])))
