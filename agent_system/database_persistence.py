...
from .database_models import (
    db_manager, ActionSelectorModel, MemoryModel, LearningSystemModel,
    GoalModel, ActionModel, ObservationModel, CrossSessionPatternModel,
    SessionModel, DecisionModel, PerformanceMetricModel, SecurityAuditModel,
    ConfigurationModel
)
...
class DatabasePersistence:
    ...
    def save_action_selector(self, selector_data: Dict[str, Any], selector_type: str = 'intelligent') -> None:
        """Save action selector data to database."""
        try:
            with self.db.get_session() as session:
                # Check if selector exists
                existing = session.query(ActionSelectorModel).filter(
                    ActionSelectorModel.selector_type == selector_type
                ).first()
    ...
    def load_action_selector(self, selector_type: str = 'intelligent') -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.db.get_session() as session:
                selector = session.query(ActionSelectorModel).filter(
                    ActionSelectorModel.selector_type == selector_type
                ).first()
            ...
            with self.db.get_session() as session:
                for memory_data in memories:
                    ...
                    existing = session.query(MemoryModel).filter(
                        MemoryModel.memory_id == memory_data.get('id', '')
                    ).first()
    ...
    def save_learning_system(self, learning_data: Dict[str, Any], system_type: str = 'default') -> None:
        """Save learning system data to database."""
        try:
            with self.db.get_session() as session:
                existing = session.query(LearningSystemModel).filter(
                    LearningSystemModel.system_type == system_type
                ).first()
    ...
    def load_learning_system(self, system_type: str = 'default') -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.db.get_session() as session:
                learning = session.query(LearningSystemModel).filter(
                    LearningSystemModel.system_type == system_type
                ).first()
        ...
        try:
            with self.db.get_session() as session:
                for goal_data in goals_data:
                    existing = session.query(GoalModel).filter(
                        GoalModel.id == goal_data.get('id', '')
                    ).first()
        ...
        try:
            with self.db.get_session() as session:
                for action_data in actions_data:
                    ...
                    existing = session.query(ActionModel).filter(
                        ActionModel.action_id == action_data.get('id', '')
                    ).first()
        ...
        try:
            with self.db.get_session() as session:
                for obs_data in observations_data:
                    ...
                    existing = session.query(ObservationModel).filter(
                        ObservationModel.observation_id == obs_data.get('id', '')
                    ).first()
    ...
    def load_observations(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db.get_session() as session:
                observations = session.query(ObservationModel).order_by(desc(ObservationModel.created_at)).all()
        ...
        try:
            with self.db.get_session() as session:
                for pattern_data in patterns_data:
                    ...
                    existing = session.query(CrossSessionPatternModel).filter(
                        CrossSessionPatternModel.pattern_id == pattern_data.get('id', '')
                    ).first()
                ...
                session.commit()
                logger.debug(f"Cross-session patterns saved to database: {len(patterns_data)} items")
    ...
    def load_cross_session_patterns(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db.get_session() as session:
                patterns = session.query(CrossSessionPatternModel).order_by(
                    desc(CrossSessionPatternModel.confidence_score)
                ).all()
    ...
    def save_configuration(self, config_key: str, config_value: Any, config_type: str = 'system') -> None:
        """Save configuration to database."""
        try:
            with self.db.get_session() as session:
                # Check if config exists
                existing = session.query(ConfigurationModel).filter(
