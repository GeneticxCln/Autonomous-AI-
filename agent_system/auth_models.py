...
from sqlalchemy import (
    ...
    ForeignKey, Index, UniqueConstraint, Table, create_engine
)
...
class DatabaseManager:
    ...
    def initialize(self):
        ...
        try:
            ...
            print(f"✅ Auth database initialized successfully: {self.database_url}")
...
class UserModel(Base):
    ...
    audit_logs = relationship("AuthSecurityEventModel", back_populates="user", cascade="all, delete-orphan")
...
DEFAULT_PERMISSIONS = [
    # Agent permissions
    {'name': 'agent.read', 'description': 'Read agent status and information', 'resource': 'agent', 'action': 'read'},
    {'name': 'agent.write', 'description': 'Modify agent configuration', 'resource': 'agent', 'action': 'write'},
    {'name': 'agent.execute', 'description': 'Execute agent operations', 'resource': 'agent', 'action': 'execute'},
    {'name': 'agent.admin', 'description': 'Full agent administration', 'resource': 'agent', 'action': 'admin'},

    # Goal permissions
    {'name': 'goals.read', 'description': 'Read agent goals', 'resource': 'goals', 'action': 'read'},
    {'name': 'goals.write', 'description': 'Create and modify goals', 'resource': 'goals', 'action': 'write'},
    {'name': 'goals.delete', 'description': 'Delete goals', 'resource': 'goals', 'action': 'delete'},

    # Action permissions
    {'name': 'actions.read', 'description': 'Read action history', 'resource': 'actions', 'action': 'read'},
    {'name': 'actions.write', 'description': 'Execute actions', 'resource': 'actions', 'action': 'write'},
    ...
    {'name': 'users.write', 'description': 'Create and modify users', 'resource': 'users', 'action': 'write'},
...
DEFAULT_ROLE_PERMISSIONS = {
    'admin': ['agent.read', 'agent.write', 'agent.execute', 'agent.admin',
              'goals.read', 'goals.write', 'goals.delete',
              'actions.read', 'actions.write',
              'system.read', 'system.write', 'system.admin',
              'users.read', 'users.write', 'users.delete', 'users.admin'],
    'manager': ['agent.read', 'agent.write', 'agent.execute',
                'goals.read', 'goals.write', 'goals.delete',
                'actions.read', 'actions.write',
...
class SecurityContext:
    ...
    def __init__(self, user: UserModel, permissions: List[str], session_id: str = None):
        ...
        self.is_admin = 'admin' in [p.split('.')[1] for p in permissions if '.' in p and p.endswith('.admin')]
