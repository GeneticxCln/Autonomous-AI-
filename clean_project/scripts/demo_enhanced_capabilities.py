...
def demo_performance_monitoring():
    ...
    for metric, level_data in current_perf["performance_levels"].items():
        print(
            f"   {metric}: {level_data['level']} ({level_data['value']:.2f}) - Target: {level_data['target']}"
        )
...
def demo_integration():
    ...
    print(
        f"   Performance Health: {status['performance_monitor_stats']['summary']['overall_health']}"
    )
    ...
    print(
        f"   Health: {perf_insights['overall_health']['overall_health']} ({perf_insights['overall_health']['score']:.1f}%)"
    )
