...
def demonstrate_enhanced_agent():
    ...
    print(
        f"   - Tool Executions: {sum(s['total_executions'] for s in status['tool_stats'].values())}"
    )
    ...
    code_goal = agent.add_goal(
        "Generate a Python script that calculates statistical measures for a dataset", priority=0.7
    )
    ...
    for tool_name, stats in final_status["tool_stats"].items():
        ...
        print(
            f"   - {tool_name}: {stats['total_executions']} executions, {success_rate:.1%} success ({tool_type})"
        )
