...
class ReasoningEngine:
    ...
    def __init__(self):
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE and NUMPY_AVAILABLE:
            ...
            try:
                ...
                logger.warning(f"Failed to load sentence transformer: {e}. Using fallback similarity")
                self.model = None
    ...
    def _load_goal_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "research": {
                ...
                "actions": ["search_information", "gather_data", "analyze_sources", "synthesize_findings"],
            ...
            "web_search": {
                ...
                "actions": ["formulate_query", "execute_search", "filter_results", "compile_findings"],
    ...
    def _extract_parameters(self, goal_description: str, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "research" in goal_description.lower() or "find" in goal_description.lower() or "search" in goal_description.lower():
            search_terms = []
            # Remove common words and extract meaningful terms
            words = goal_description.split()
            for word in words:
                if len(word) > 3 and word.lower() not in ["the", "and", "for", "with", "from", "about"]:
        ...
        for i, action_name in enumerate(actions):
            if action_name in self.action_templates:
                ...
                action = {
                    ...
                    "parameters": self._customize_parameters(template["parameters"], parameters, action_name),
    ...
    def _customize_parameters(self, base_params: Dict[str, Any], goal_params: Dict[str, Any], action_name: str) -> Dict[str, Any]:
...
"""
    ...
    def learn_from_episode(self, goal_analysis: Dict[str, Any], actions: List[Dict], success_score: float):
    ...
    def calculate_similarity_score(self, goal_text: str, pattern_name: str) -> float:
        ...
        try:
            ...
            if NUMPY_AVAILABLE:
                ...
                similarity = dot_product / (norm_goal * norm_pattern) if norm_goal * norm_pattern > 0 else 0
    ...
    def enhanced_analyze_goal(self, goal_description: str) -> Dict[str, Any]:
        ...
        if self.model:
            for pattern_name in self.goal_patterns.keys():
                semantic_scores[pattern_name] = self.calculate_similarity_score(goal_description, pattern_name)
        ...
        if combined_scores:
            ...
            if score_info['combined_score'] > 0.1:  # Minimum confidence threshold
                ...
                return {
                    ...
                    "explanation": self._generate_analysis_explanation(goal_description, pattern_name, score_info)
                }
    ...
    def _combine_matching_methods(self, keyword_scores: Dict[str, float], semantic_scores: Dict[str, float]) -> Dict[str, Dict[str, float]]:
    ...
    def _generate_analysis_explanation(self, goal_text: str, pattern_name: str, score_info: Dict[str, float]) -> str:
        ...
        return f"Goal matched to '{pattern_name}' pattern with {confidence_level} confidence. " \
