...
class AIPlanner:
    ...
    def create_intelligent_plan(
        ...
        logger.info(
            f"Created plan with {len(optimized_actions)} actions, confidence: {confidence:.2f}"
        )
    ...
    def _find_alternative_tool(
        self, original_tool: str, available_tools: List[str]
    ) -> Optional[str]:
    ...
    def refine_plan_based_on_feedback(self, original_plan: Plan, feedback: Dict[str, Any]) -> Plan:
        ...
        refined_plan = Plan(
            ...
            * 0.9,  # Slightly reduce confidence for refined plans
        )
    ...
    def _create_alternative_action(self, failed_action: Action) -> Optional[Action]:
        ...
        if failed_action.tool_name == "web_search":
            ...
            return Action(
                ...
                parameters={
                    "task": f"Offline analysis of: {failed_action.parameters.get('query', 'unknown')}"
                },
...
class AIHierarchicalPlanner(AIPlanner):
    ...
    def _create_fallback_plan(self, goal: Goal, available_tools: List[str]) -> Plan:
        ...
        return Plan(goal_id=goal.id, actions=[simple_action], estimated_cost=0.5, confidence=0.3)
    ...
    def _create_simple_plan(self, goal: Goal, available_tools: List[str]) -> Plan:
        ...
        if any(word in goal_lower for word in ["research", "find", "search"]):
            ...
            action = Action(
                ...
                name="generic_task",
                tool_name=(
                    "generic_tool" if "generic_tool" in available_tools else available_tools[0]
                ),
                parameters={"goal": goal.description},
