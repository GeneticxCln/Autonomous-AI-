...
class SystemHealthMonitor:
    ...
    def get_comprehensive_health_check(self) -> HealthMetrics:
        ...
        try:
            ...
            recommendations = self._generate_health_recommendations(
                component_health, performance_summary, security_status
            )
    ...
    def _identify_active_issues(self, component_scores: Dict[str, float]) -> List[str]:
        ...
        for component, score in component_scores.items():
            if score < 0.3:
                issues.append(
                    f"Critical issue in {component.replace('_', ' ')} (score: {score:.2f})"
                )
            elif score < 0.6:
                issues.append(
                    f"Performance issue in {component.replace('_', ' ')} (score: {score:.2f})"
                )
    ...
    def _generate_health_recommendations(
            ...
            if score < 0.3:
                if component == "ai_performance":
                    recommendations.append(
                        "Review AI performance metrics and optimize slow components"
                    )
                elif component == "memory_optimization":
                    recommendations.append(
                        "Increase memory cleanup frequency and optimize memory usage"
                    )
        ...
        try:
            ...
            if "response_times" in performance_summary:
                ...
                if avg_response > 1.0:
                    recommendations.append(
                        "Slow response times - consider implementing caching and optimization"
                    )
        ...
        try:
            ...
            if failed_audits > 0:
                recommendations.append(
                    "Address security validation failures and improve input sanitization"
                )
        ...
        if overall_score < 0.6:
            recommendations.append(
                "Overall system health is poor - perform comprehensive system review"
            )
