...
class InputValidator:
    ...
    def validate_file_path(self, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            if (
                path_obj.suffix
                and path_obj.suffix.lower() not in self.config.allowed_file_extensions
            ):
                return ValidationResult(
                    False,
                    SecurityLevel.HIGH,
    ...
    def validate_shell_command(self, command: str) -> ValidationResult:
        ...
        try:
            ...
            if (
                command_parts
                and command_parts[0]
                not in self.validation_rules["shell_command"]["allowed_commands"]
            ):
                return ValidationResult(
                    False,
                    SecurityLevel.HIGH,
...
class ResourceValidator:
    ...
    def validate_memory_usage(self, memory_mb: float) -> ValidationResult:
        ...
        return ValidationResult(
            True, SecurityLevel.LOW, f"Memory usage ({memory_mb}MB) within limits"
        )
    ...
    def validate_file_size(self, file_size_mb: float) -> ValidationResult:
        ...
        return ValidationResult(
            True, SecurityLevel.LOW, f"File size ({file_size_mb}MB) within limits"
        )
    ...
    def validate_execution_time(self, execution_time: float) -> ValidationResult:
        ...
        return ValidationResult(
            True, SecurityLevel.LOW, f"Execution time ({execution_time}s) within limits"
        )
    ...
    def _generate_security_recommendations(self) -> List[str]:
        ...
        if failed_count / total_count > 0.1:
            recommendations.append(
                "High failure rate detected. Review and improve input validation."
            )

        # Check for patterns
        operation_types = [
            audit["operation_type"] for audit in self.audit_log if not audit["is_valid"]
        ]
        if operation_types:
