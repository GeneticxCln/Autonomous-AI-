...
from .models import Action, Goal, ActionStatus
...
class IntelligentActionSelector:
    ...
    def select_action(
        ...
        logger.info(f"Selecting action from {len(available_actions)} options for goal: {goal.description}")
    ...
    def _evaluate_parameter_relevance(self, parameters: Dict[str, Any], context: Dict[str, Any]) -> float:
        ...
        if "query" in parameters and "search_terms" in context:
            common_terms = set(str(parameters["query"]).lower().split()) & set(context["search_terms"])
            if common_terms:
    ...
    def _calculate_prerequisite_score(self, action: Action, completed_actions: List[str]) -> float:
        ...
        completed_prerequisites = [prereq for prereq in prerequisites if prereq in completed_actions]
    ...
    def _store_selection_rationale(self, action: Action, goal: Goal, context: Dict[str, Any], score: float):
    ...
    def get_action_recommendations(self, goal: Goal, available_actions: List[Action]) -> List[Tuple[Action, float]]:
    ...
    def learn_from_success_pattern(self, goal: Goal, successful_actions: List[Action], success_score: float):
