...
from .models import Action, Goal, ActionStatus, Observation
...
class IntelligentObservationAnalyzer:
    ...
    def _load_success_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "high_success": [
                "completed successfully", "task finished", "goal achieved",
                "all objectives met", "100% complete"
            ],
            "medium_success": [
                "partially completed", "significant progress", "major objectives met",
                "approaching completion"
            ],
            "low_success": [
                "minimal progress", "preliminary results", "initial stage",
                "requires additional work"
            ],
            "failure": [
                "failed", "error", "unsuccessful", "cannot complete",
                "insufficient data", "invalid input"
            ]
        }
    ...
    def analyze_observation(
        ...
        replanning_needed = self._determine_replanning_need(observation, success_score, goal_progress)
        ...
        analysis = {
            ...
            "insights": insights,
            "anomalies": anomalies,
            "recommendations": self._generate_recommendations(observation, outcome_type, success_score),
            "metadata": {
    ...
    def _calculate_success_score(self, observation: Observation, expected_outcome: str, goal: Goal) -> float:
    ...
    def _assess_goal_progress(self, observation: Observation, expected_outcome: str, goal: Goal) -> float:
    ...
    def _determine_replanning_need(self, observation: Observation, success_score: float, goal_progress: float) -> bool:
    ...
    def _generate_insights(self, observation: Observation, outcome_type: str, success_score: float, goal: Goal) -> List[str]:
    ...
    def _detect_anomalies(self, observation: Observation, expected_outcome: str, goal: Goal) -> List[str]:
    ...
    def _generate_recommendations(self, observation: Observation, outcome_type: str, success_score: float) -> List[str]:
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        success_count = sum(1 for data in self.learning_data.values() 
                           if data["analysis"]["success_score"] > 0.7)
        
        avg_success_score = sum(data["analysis"]["success_score"] 
                               for data in self.learning_data.values()) / total_analyses
